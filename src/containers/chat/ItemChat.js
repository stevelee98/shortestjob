...
class ItemChat extends PureComponent {
    ...
    render() {
        const { data, index, userAdminId, onPressSendAction, resourceUrlPath } = this.props;
        let parseItem = {
        ...
        if (parseItem.messageType == messageType.IMAGE_MESSAGE) {
            String(parseItem.content).split(',').forEach(pathImage => {
                this.imageUrls.push({ path: resourceUrlPath + "/" + pathImage + '?token=' + parseItem.tokenImage })
            });
        }
        return (
    ...
    renderListImage = (parseItem) => {
        return (
            <View style={[{
                ...
                {parseItem.from == this.props.userAdminId && this.renderAcceptImage(parseItem)}
            </View>
        )
    }
    ...
    renderItemChat = (parseItem) => {
        if (!Utils.isNull(this.props.seller)) {
            if (!Utils.isNull(this.props.seller.avatarPath)) {
                var avatarSeller = this.props.seller.avatarPath.indexOf("http") != -1 ? this.props.seller.avatarPath :
                    (this.props.resourceUrlPath + '/' + this.props.seller.avatarPath)
            } else { var avatarSeller = '' }
        } else { var avatarSeller = '' }
        return (
            <View style={[commonStyles.viewHorizontal, {
                flex: 0,
                ...
                <View style={[commonStyles.viewHorizontal, { flex: 0, alignItems: "flex-end" }]} >
                    <TouchableOpacity
                        onPress={() => {
                            this.props.navigation.navigate('InfoSeller', {
                                ...
                                item: this.props.seller,
                                avatarSeller: avatarSeller,
                                resourceUrlPathResize: this.props.resourceUrlPathResize,
                    ...
                    <View style={[parseItem.from !== this.props.userId ? styles.member : styles.user]} >
                        <Text style={[commonStyles.text, { fontSize: Fonts.FONT_SIZE_MEDIUM, maxWidth: width * 0.65, color: parseItem.from !== this.props.userId ? Colors.COLOR_TEXT : Colors.COLOR_WHITE }]}>
                            {parseItem.content}
                        </Text>
        ...
        return (
            ...
            && <View style={{ position: "relative" }}>
                <TouchableOpacity
                    ...
                    style={{ flexDirection: 'row', alignItems: 'center', ...this.getStyleImage(index) }}>
                    <ImageLoader
