...
export default class DropdownAlert extends Component {
  ...
  static defaultProps = {
    ...
    confirmBtnImageSrc:require('./assets/agree.png'),
    infoColor: '#2B73B6',
    warnColor: '#cd853f',
    errorColor: '#cc3232',
    successColor: '#32A54A',
  ...
  renderConfirm(show){
    if(show){
      if(this.props.renderConfirm){
        return this.props.renderConfirm(this.props);
      } else {
        return(
          <TouchableOpacity
            style={{
            ...
            onPress={() => this.props.onConfirm(this.props)}
          >
            <ImageView style={this.props.cancelBtnImageStyle} source={this.props.confirmBtnImageSrc} resizeMode={'contain'}/>
          </TouchableOpacity>
        )
      }
    }
  ...
  render() {
    ...
    if (isOpen) {
      ...
      let { activeStatusBarBackgroundColor, translucent, updateStatusBar, activeStatusBarStyle, cancelBtnImageSrc, showCancel,showConfirm } = this.props;
      if (IS_ANDROID) {
        if (translucent) {
      ...
      return (
        <Animated.View ref={ref => this.mainView = ref} {...this._panResponder.panHandlers} style={wrapperStyle}>
          <TouchableOpacity
            activeOpacity={!this.props.tapToCloseEnabled || showConfirm ? 1 : 0.95}
          ...
          >
            <View style={style}>
              ...
              {this.renderConfirm(showConfirm)}
            </View>
          </TouchableOpacity>
        </Animated.View>
      );
